...
task_path="${BRUNOST_JUDGE_CANARY_TASK:-examples/deterministic-sum}"
submission_path="${BRUNOST_JUDGE_CANARY_SUBMISSION:-examples/canary-deterministic-sum}"
canary_args=(
  --url "${BRUNOST_JUDGE_URL:-http://127.0.0.1:8787}"
  --token "${BRUNOST_JUDGE_API_TOKEN:-}"
  --task-path "$task_path"
  --submission "$submission_path"
  --task-ref "${BRUNOST_JUDGE_CANARY_TASK_REF:-canary/deterministic-sum-v1}"
